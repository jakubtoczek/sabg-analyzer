...
def _tile_boxes_px(tiles_global, scene, sc, W, H):
    """Map global acquired-tile rects to canvas-pixel boxes (ox, oy, ex, ey), clipped."""
    out = []
    for X, Y, w, h in tiles_global:
        ox = max(0, int(round((X - scene.x) * sc))); oy = max(0, int(round((Y - scene.y) * sc)))
        ex = min(W, int(round((X + w - scene.x) * sc))); ey = min(H, int(round((Y + h - scene.y) * sc)))
        if ex > ox and ey > oy:
            out.append((ox, oy, ex, ey))
    return out
...
def _build_config_snapshot(cfg: Config, rows: list[dict]) -> dict:
    ...
    snapshot = {
        ...
        "gui": {"info_opens": list(cfg.gui.info_opens),
                ...
                "info_columns": list(cfg.gui.info_columns)},
...
def _export_snapshot(overrides: dict) -> dict:
    ...
    base = {"fov_um": d.fov_um, "scalebar_um": d.scalebar_um,
            ...
            "min_tissue_frac": d.min_tissue_frac,
            "fov_select_mode": d.fov_select_mode,
            "max_artifact_frac": d.max_artifact_frac, "max_fold_frac": d.max_fold_frac,
            "wb": d.wb, "raw": d.raw,
            "plain": d.plain, "plain_bases": list(d.plain_bases),
            "qc_overlay": d.qc_overlay,
